<header class="flex items-center justify-between px-4 py-2 border-b border-[#303030] shadow-sm sticky top-0 bg-[#181818] text-white z-50">
    ...
    <div class="flex items-center space-x-4 relative" id="navbar-user">
        <?php if (isset($_SESSION['user'])): ?>
            <button onclick="loadComponent('upload')" class="px-3 py-2 rounded-full font-medium border border-white text-white hover:bg-white hover:text-black transition">
                Upload
            </button>

            <button onclick="toggleDropdown()" class="flex items-center space-x-1 text-sm font-medium text-white hover:text-gray-300">
...
document.addEventListener('click', function (e) {
    ...
    const btn = document.querySelector('#navbar-user button');
    if (!menu || !btn) return;
